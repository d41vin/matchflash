...
- A dedicated Railway Node worker owns TxLINE Mainnet level-12 score and odds SSE connections, credentials, reconnect behavior, gzip handling, heartbeat filtering, and `Last-Event-ID` resume state. It writes to a narrowly authenticated ingestion boundary that invokes internal Convex processing; browser code never calls TxLINE and sensitive writes are not exposed as general public mutations.
