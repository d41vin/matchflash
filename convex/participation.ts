...
function profileForWallet(walletAddress: string) {
  ...
  const colorIndex = walletAddress
    .split("")
    .reduce((sum, character) => sum + character.charCodeAt(0), 0) % REACTION_COLORS.length
...
function database(ctx: MutationCtx): GenericDatabaseWriter<MatchFlashDataModel> {
...
export const recordLiveReaction = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!isLiveFixturePhase(effectiveFixturePhase(args.fixtureId, state?.phase))) {
